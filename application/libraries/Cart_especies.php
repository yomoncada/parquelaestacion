...
class Cart_Especies {
	 ...
	 * @var array
	 */
	protected $_especies_contents = array();
	...
	public function __construct($params = array())
	{
		...
		$this->_especies_contents = $this->CI->session->userdata('especies_contents');
		if ($this->_especies_contents === NULL)
		{
			// No cart exists so we'll set some base values
			$this->_especies_contents = array('cart_total' => 0, 'total_especies' => 0);
		}
	...
	protected function _insert($especies = array())
	{
        ...
        if($contador<1)
        {
			$this->_especies_contents[$rowid] = $especies;
        }
        return $rowid;
	}
	...
	protected function _update($especies = array())
	{
		...
		if ( ! isset($especies['rowid'], $this->_especies_contents[$especies['rowid']]))
		{
			return FALSE;
		}
		...
		if (isset($especies['cantidad']))
		{
			...
			if ($especies['cantidad'] == 0)
			{
				unset($this->_especies_contents[$especies['rowid']]);
				return TRUE;
			}
		}

		// find updatable keys
		$keys = array_intersect(array_keys($this->_especies_contents[$especies['rowid']]), array_keys($especies));

		// product id & nombre shouldn't be changed
		foreach (array_diff($keys, array('id', 'nombre')) as $key)
		{
			$this->_especies_contents[$especies['rowid']][$key] = $especies[$key];
		}
	...
	protected function _save_cart()
	{
		...
		$this->_especies_contents['total_especies'] = $this->_especies_contents['cart_total'] = 0;
		foreach ($this->_especies_contents as $key => $val)
		{
			...
			$this->_especies_contents['total_especies'] += $val['cantidad'];
		}

		// Is our cart empty? If so we delete it from the session
		if (count($this->_especies_contents) <= 2)
		{
		...
		$this->CI->session->set_userdata(array('especies_contents' => $this->_especies_contents));
	...
	{
		return $this->_especies_contents['cart_total'];
	}
	 ...
	 */
	 ...
	 {
		// unset & save
		unset($this->_especies_contents[$rowid]);
		$this->_save_cart();
		return TRUE;
	 }
	...
	{
		return $this->_especies_contents['total_especies'];
	}
	...
	public function contents($newest_first = FALSE)
	{
		...
		$cart = ($newest_first) ? array_reverse($this->_especies_contents) : $this->_especies_contents;

		// Remove these so they don't create a problem when showing the cart table
		unset($cart['total_especies']);
	...
	{
		return (in_array($row_id, array('total_especies', 'cart_total'), TRUE) OR ! isset($this->_especies_contents[$row_id]))
			? FALSE
			: $this->_especies_contents[$row_id];
	}
	...
	{
		return (isset($this->_especies_contents[$row_id]['options']) && count($this->_especies_contents[$row_id]['options']) !== 0);
	}
	...
	{
		return isset($this->_especies_contents[$row_id]['options']) ? $this->_especies_contents[$row_id]['options'] : array();
	}
