...
class Censo extends CI_Controller
{
    ...
    public function list_censos()
    {
        $list = $this->censo->get_datatables();
        $data = array();
        $no = $_POST['start'];
        $i = 1;
        ...
        $output = array(
            ...
            "recordsFiltered" => $this->censo->count_filtered(),
            "data" => $data
        );
    ...
    public function process($id_cen = NULL)
    {
        if($this->cart_empleados->contents() != NULL && $this->cart_areas->contents() != NULL && $this->cart_especies->contents() != NULL && $this->cart_implementos->contents() != NULL && $this->cart_actividades->contents() != NULL)
        {
            ...
            if($this->form_validation->run() == TRUE)
            {
                ...
                $bitacora = array(
                    'tipo' => 'Censo',
                    'movimiento' => 'Se ha registrado un censo.',
                    'usuario' => $this->session->userdata('id_usuario')
                );      
    ...
    public function update($id_cen = NULL)
    {
        if($this->cart_empleados->contents() != NULL && $this->cart_areas->contents() != NULL && $this->cart_especies->contents() != NULL && $this->cart_implementos->contents() != NULL && $this->cart_actividades->contents() != NULL)
        {
            ...
            $bitacora = array(
                    'tipo' => 'Censo',
                    'movimiento' => 'Se ha actualizado el estado del censo '.$id_cen.'.',
                    'usuario' => $this->session->userdata('id_usuario')
                );      
